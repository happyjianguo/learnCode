...
import cn.com.jansh.util.AppException;
...
public class GlobalExceptionResolver extends AbstractHandlerExceptionResolver {
	 ...
	 * present. A value of "ServletException" would match {@code
	 ...
	 * define a rule for all checked exceptions. With more unusual exception
	 ...
	 * {@link #setStatusCodes(Properties)}. Keys are view names; values are
	 * status codes.
	 */
	 ...
	 *            {@link HttpServletResponse#SC_INTERNAL_SERVER_ERROR}) or 404 (
	 ...
	 * <p>
	 * This can be either set to a different attribute name or to {@code null}
	 ...
	 * the {@link #setExcludedExceptions(Class[]) "excludedExecptions"}, then
	 * searching the {@link #setExceptionMappings "exceptionMappings"}, and
	 * finally using the {@link #setDefaultErrorView "defaultErrorView"} as a
	 * fallback.
	 * 
	 * @param ex
	 *            the exception that got thrown during handler execution
	 ...
	 * <p>
	 * The default implementation returns the status code for the given view
	 * name (specified through the {@link #setStatusCodes(Properties)
	 * statusCodes} property), or falls back to the {@link #setDefaultStatusCode
	 ...
	 * @return the HTTP status code to use, or {@code null} for the servlet
	 *         container's default (200 in case of a standard error view)
