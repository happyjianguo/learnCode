...
import cn.com.jansh.dao.mapper.WxbAccessTokenMapper;
import cn.com.jansh.util.HttpClientUtil;
public class AccessTokenTicket {

	private static final Logger logger = LogManager.getLogger(AccessTokenTicket.class);

	@Autowired
	private WxbAccessTokenMapper accessTokenMapper;
	...
	private String getAccessTokenRemote(String appid) throws Exception {
		...
		try {
			accessTokenMap = mapper.readValue(accessTokenJson, new TypeReference<Map<String, String>>() {
			});
		} catch (Exception e) {
	...
	private String getTicketRemote(String appid) throws Exception {
		...
		try {
			ticketMap = mapper.readValue(ticketJson, new TypeReference<Map<String, String>>() {
			});
		} catch (Exception e) {
			logger.error("accessTokenJson报文返回格式错误：{}", e);
			throw e;
