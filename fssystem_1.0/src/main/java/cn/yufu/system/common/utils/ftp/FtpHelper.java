...
public class FtpHelper implements Closeable {
	 ...
	 * @return true||false
	 */
	public boolean uploadFile(String localFileName, String ftpDirName,
			String ftpFileName) {
	...
	public boolean uploadFile(String localFileName, String ftpDirName,
		...
		Debug.printFormat("准备上传 [{0}] 到 ftp://{1}/{2}", localFileName,
				ftpDirName, ftpFileName);
		...
		try (FileInputStream fis = new FileInputStream(srcFile)) {
			...
			if (onUploadFileAfter != null) {
				onUploadFileAfter.apply(new FtpFileInfo(localFileName,
						ftpDirName, ftpFileName));
			}
			// 删除文件
			if (deleteLocalFile) {
	 ...
	 * @author xxj
	 */
	public boolean downloadFile(String ftpDirName, String ftpFileName,
		...
		try {
			...
			System.out
					.println("下载ftp文件失败：" + ftpFileName + ";目录：" + ftpDirName);
			return false;
		} catch (Exception e) {
			e.printStackTrace();
			return false;
