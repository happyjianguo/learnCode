 ...
 */
package cn.yufu.system.common.persistence.dialect.db;
...
public class SQLServerDialect implements Dialect {
    ...
    public String getLimit(String sql, int offset, int limit) {
		...
		pagingSelect.append("SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY ID DESC) AS rowid, * FROM ( ");
		pagingSelect.append(sql);
		int end = offset + limit;
		pagingSelect.append(") t WHERE t.rowid > " + offset + " AND t.rowid <=" + end);
