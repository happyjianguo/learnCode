...
<script type="text/javascript">
	...
	function exportExcel(flag) {
		$("#form").attr("action","${pageContext.request.contextPath}/MerStlFinalBook/exportExcel?flagOnline="+flag);
		$("#form").submit();
	}
	function query(flag){
		$("#form").attr("action","${pageContext.request.contextPath}/MerStlFinalBook/getList?flagOnline="+flag);
	...
	function batchPrint() {
		var url = '${pageContext.request.contextPath}/MerStlFinalBook/batchPrint?merNo='+$("#merNo").val()+"&merName="+$("#merName").val()+
		"&stlDate="+$("#stlDate").val()+"&payoutDate="+$("#payoutDate").val()+"&payoutJunl="+$("#payoutJunl").val()+
		"&seqNo="+$("#seqNo").val()+"&payoutStatus="+$("#payoutStatus").val()+ '&random='+ Math.random()
    ...
    var shenhe =function (type) { 
        ...
        url='${pageContext.request.contextPath}/MerStlFinalBook/shenhe';
		$.ajax({
			url : url,
			type : "post",
			data : {
    ...
    var getCityList = function() {
    	$.ajax({
    		url : '${pageContext.request.contextPath}/MerStlFinalBook/getAreaListByFid',
    		type : "post",
    		data : {
		...
		<div class="shm_contab">
			<div class="poup_right">
				<div class="shmcr_cont01">
					<div class="sh_title">结算终表</div>
					<div class="shmc_tab2">
						<form id="form" action="${pageContext.request.contextPath}/MerStlFinalBook/getList" method="post">
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<tr>																	
									<td style="text-align: right;" colspan="8">
										...
										<shiro:hasPermission name="fssystem:merStlFinalBook:shenhe">
											<c:if test="${query.payoutStatus=='0'}">
												<input type="button" value="审核通过" class="button" onClick="shenhe('0')"/>	
											</c:if>
										</shiro:hasPermission>
										<shiro:hasPermission name="fssystem:merStlFinalBook:dakuan">
											<c:if test="${query.payoutStatus=='1'}">
												<input type="button" value="打款成功" class="button" onClick="shenhe('1')"/>	
											</c:if>
										</shiro:hasPermission>
										<shiro:hasPermission name="fssystem:merStlFinalBook:zuofei">
											<c:if test="${query.payoutStatus=='2'}">
												<input type="button" value="作废" class="button" onClick="shenhe('2')"/>	
											</c:if>		
										</shiro:hasPermission>																																					
									</td>	
								</tr>
						...
						<table width="100%" border="1" cellspacing="0" cellpadding="0">
							...
							<c:if test="${ ClearMerStlFinalBookList != null  }">
								...
								<tr>
									<td colspan="22" class="page">${pageBar}</td>
								</tr>
							</c:if>
							<c:if test="${ ClearMerStlFinalBookList == null  }">
								<tr>
