...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#query').click(function() {
			$("#form").attr("action","${pageContext.request.contextPath}/ViewAccnoAvlBal/getList");
			$("#form").submit();
		});		
		
		$('#export').click(function() {
			$("#form").attr("action","${pageContext.request.contextPath}/ViewAccnoAvlBal/exportExcel");
			$("#form").submit();
		});	
			...
			<div class="shm_contab">
				<div class="poup_right">
					<div class="shmcr_cont01">
						...
						<div class="shmc_tab2">
							<form id="form" action="${pageContext.request.contextPath}/ViewAccnoAvlBal/getList"
								method="post">
								<input type="hidden" id="curPage" name="curPage" value="${curPage}" />
								<input type="hidden" id="pageSize" name="pageSize" value="${pageSize}" />
								<input type="hidden" id="flag" name="flag" value="${query.flag}" />
							...
							<table width="100%" border="1" cellspacing="0" cellpadding="0">
								...
								<c:if test="${viewAccnoAvlBalList != null}">
									<c:forEach items="${viewAccnoAvlBalList}" var="info"
										varStatus="index">
										<tr class="cow">
											<td>${((curPage-1) * pageSize) + index.count }</td>
											<td>${info.cardbin}</td>
											<td>
												<fmt:formatNumber value="${info.cardNum}" pattern="#,###,###,###,###,###"
									...
									<c:if test="${fn:length(viewAccnoAvlBalList)==0}">
