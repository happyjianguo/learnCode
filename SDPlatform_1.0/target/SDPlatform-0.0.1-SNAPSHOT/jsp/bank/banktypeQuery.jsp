			...
			<td align="center">
				...
				<html:form action="/banktype" method="post">
					...
					<display:table name="pageInfoResult.resultItems" partialList="true" size="pageInfoResult.totalCount" pagesize="8" class="dpTable" id="displayTable" requestURI="/banktype.do">
						...
						<display:column title="类型编码" style="width:10%;text-align:center" property="bankType" headerClass="sortable" sortable="true" />
						<display:column title="类型名称" style="width:20%;text-align:center" property="typeName" headerClass="sortable" sortable="true" />
						<display:column title="英文名称" style="width:15%;text-align:center" property="typeEname" headerClass="sortable" sortable="true" />
						<display:column title="操作" style="width:10%;text-align:center">
						<shiro:hasPermission name="posp:banktype:edit">
							<a href="#" onclick="editClicks('<c:out value="${displayTable.bankType }"/>','findItem')">编辑</a>
						</shiro:hasPermission>
						</display:column>
					</display:table>
					<!-- 用Diaplay Tag来显示 -->		
