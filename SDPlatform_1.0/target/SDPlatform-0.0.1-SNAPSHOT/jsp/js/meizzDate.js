...
var _VersionInfo="tongfangsoft 1.0"	//版本信息

//==================================================== WEB 页面显示部分 =====================================================
var strFrame;		//存放日历层的HTML代码
document.writeln('<iframe src=blank.html id=meizzDateLayer Author=wayx frameborder=0 style="position: absolute; width: 144; height: 211; z-index: 9998; display: none"></iframe>');
strFrame='<style>';
strFrame+='INPUT.button{BORDER-RIGHT: #5DBCF9 1px solid;BORDER-TOP: #5DBCF9 1px solid;BORDER-LEFT: #5DBCF9 1px solid;';
strFrame+='BORDER-BOTTOM: #5DBCF9 1px solid;BACKGROUND-COLOR: #fff8ec;font-family:宋体;}';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=160 bordercolor=#5DBCF9 bgcolor=#5DBCF9 Author="wayx">';
...
strFrame+='      <tr align=center Author="wayx"><td width=16 align=center bgcolor=#5DBCF9 style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='         onMouseDown="parent.startPrev();" onMouseUp="parent.stopPrev();" title="向前翻 1 月" Author=meizz><b Author=meizz>&lt;</b>';
strFrame+='        </td><td width=60 align=center style="font-size:12px;cursor:default" Author=meizz ';
strFrame+='onmouseover="style.backgroundColor=\'#B2CDFB\'" onmouseout="style.backgroundColor=\'white\'" ';
...
strFrame+='<td width=48 align=center style="font-size:12px;cursor:default" Author=meizz onmouseover="style.backgroundColor=\'#B2CDFB\'" ';
strFrame+=' onmouseout="style.backgroundColor=\'white\'" onclick="parent.tmpSelectMonthInnerHTML(this.innerText.length==3?this.innerText.substring(0,1):this.innerText.substring(0,2))"';
strFrame+='        title="点击这里选择月份"><span id=meizzMonthHead Author=meizz></span></td>';
strFrame+='        <td width=16 bgcolor=#5DBCF9 align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='         onMouseDown="parent.startNext();" onMouseUp="parent.stopNext();" title="向后翻 1 月" Author=meizz><b Author=meizz>&gt;</b></td></tr>';
strFrame+='    </table></td></tr>';
strFrame+='  <tr Author="wayx"><td width=142 height=18 Author="wayx">';
strFrame+='<table border=1 cellspacing=0 cellpadding=0 bgcolor=#5DBCF9 ' + (bMoveable? 'onmousedown="DragStart()" onmouseup="DragEnd()"':'');
strFrame+=' BORDERCOLORLIGHT=#5DBCF9 BORDERCOLORDARK=#FFFFFF width=140 height=20 Author="wayx" style="cursor:' + (bMoveable ? 'move':'default') + '">';
...
strFrame+='    <table border=1 cellspacing=2 cellpadding=0 BORDERCOLORLIGHT=#5DBCF9 BORDERCOLORDARK=#FFFFFF bgcolor=#fff8ec width=140 height=120 Author="wayx">';
...
strFrame+='          <tr Author="wayx"><td Author=meizz align=left><input Author=meizz type=button class=button value="<<" title="向前翻 1 年"  onMouseDown="parent.startPrevY();" onMouseUp="parent.stopPrevY();" ';
strFrame+='             onfocus="this.blur()" style="font-size: 12px; height: 20px"><input Author=meizz class=button title="向前翻 1 月" type=button ';
strFrame+='             value="< "  onMouseDown="parent.startPrev();" onMouseUp="parent.stopPrev();" onfocus="this.blur()" style="font-size: 12px; height: 20px"></td><td ';
strFrame+='             Author=meizz align=center><input Author=meizz type=button class=button value=当前 onclick="parent.meizzToday()" ';
strFrame+='             onfocus="this.blur()" title="当前日期" style="font-size: 12px; height: 20px; cursor:hand"></td><td ';
strFrame+='             Author=meizz align=right><input Author=meizz type=button class=button value=" >" onMouseDown="parent.startNext();" onMouseUp="parent.stopNext();" ';
strFrame+='             onfocus="this.blur()" title="向后翻 1 月" class=button style="font-size: 12px; height: 20px"><input ';
strFrame+='             Author=meizz type=button class=button value=">>" title="向后翻 1 年" onMouseDown="parent.startNextY();" onMouseUp="parent.stopNextY();" ';
...
*/
	...
	if(tleft+160>pageX+pageW){
		tleft=pageW-160;
		if(tleft<0){
			tleft=0;
		}
	}
	if(ttop+thei+211>pageH+pageY){
		ttop=ttop-215-thei;
		if(ttop<0){
			ttop=0;
		}
...
function calenderDocumentOnClick() //任意点击时关闭该控件	//ie6的情况可以由下面的切换焦点处理代替
{
  with(window.event)
...
document.attachEvent("onclick",calenderDocumentOnClick);

function calenderDocumentKeyUp()		//按Esc键关闭，切换焦点关闭
  {
...
document.attachEvent("onkeyup",calenderDocumentKeyUp);
...
function meizzToday()  //Today Button
  {
	...
	strmeizzTheMonth="0"+meizzTheMonth;
	strmeizzTheMonth=strmeizzTheMonth.substring(strmeizzTheMonth.length-2,strmeizzTheMonth.length);
    today=new Date().getDate();
	strtoday="0"+today;
	strtoday=strtoday.substring(strtoday.length-2,strtoday.length);
    //meizzSetDay(meizzTheYear,meizzTheMonth);
    if(outObject){
		outObject.value=meizzTheYear + "-" + strmeizzTheMonth + "-" + strtoday;
    }
    closeLayer();
  }
    ...
    if (meizzWDay[i]!="")
      {
		...
		if(i<firstday)		//上个月的部分
		{
			...
			else
			{
				...
				meizzWDay[i] == new Date().getDate()) ? "#B2CDFB":"#e0e0e0");
				//将选中的日期显示为凹下去
				if((mm==1?yy-1:yy)==outDate.getFullYear() && (mm==1?12:mm-1)== outDate.getMonth() + 1 &&
				meizzWDay[i]==outDate.getDate())
				{
					da.borderColorLight="#FFFFFF";
					da.borderColorDark="#5DBCF9";
				}
			}
		}
