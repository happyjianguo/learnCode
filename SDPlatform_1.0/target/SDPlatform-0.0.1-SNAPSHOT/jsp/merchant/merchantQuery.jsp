			...
			<td align="center">
				<html:form action="/merchant" method="post">
					...
					<display:table name="pageInfoResult.resultItems" partialList="true" size="pageInfoResult.totalCount" pagesize="8" class="dpTable" id="displayTable" requestURI="/merchant.do">
						...
						<display:column title="商户编号" style="width:13%;text-align:center" property="merchantId" headerClass="sortable" sortable="true" />
						...
						<c:if test='${displayTable.merchantStat eq "Y"}'>正常</c:if>
						<c:if test='${displayTable.merchantStat eq "N"}'>冻结</c:if>
						</display:column>
						<display:column title="行号" style="width:8%;text-align:center" property="signBankId" headerClass="sortable" sortable="true" />
						<display:column title="主机号" style="width:6%;text-align:center" property="signHostId" headerClass="sortable" sortable="true" />
