...
<head>
	...
	function init()
	{
		<c:if test="${!empty param._businessid}" >
		var _businessid = '<c:out value="${param._businessid}"/>';
		document.forms[0]._businessid.value=_businessid;
		</c:if>	
	}
	...
	function editClicks(params,method)
	{
		...
		document.forms[0].businessid.value=params;
		document.forms[0].submit();
		
	}
	function resetClick1(){
		document.forms[0]._businessid.value="";
	}
	</script>
	
</head>

<shiro:lacksPermission name="posp:tabBusinessRates:view">
    <div class="permission_error">用户[${fns:getUserName()}]没有该权限<br/></div>
</shiro:lacksPermission>
<shiro:hasPermission name="posp:tabBusinessRates:view">
...
<body onload="init();">
	<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" class="table1_border">
		...
		<tr>
			<td align="center">
				<html:form action="/tabBusinessRates" method="post">
					...
					<table width="96%" border="0" align="center" cellpadding="0" cellspacing="1" class="table2_border">
						<tr>
							...
							<td class="table2_td" width="40%">
								<input name="_businessid" type="text"  maxlength="3"  style="width:135px;">
							</td>
							<td class="table2_td_title" width="10%">  
					...
					<!-- 用Diaplay Tag来显示 -->
					<display:table name="pageInfoResult.resultItems" partialList="true" size="pageInfoResult.totalCount" pagesize="8" class="dpTable" id="displayTable" requestURI="/tabBusinessRates.do">
						<display:column title="选择" style="width:5%;text-align:center">
							
						<input type="checkbox" name="checkItem" value="<c:out value="${displayTable.businessid}"/>">
						
						</display:column>
						<display:column title="业务ID" style="width:15%;text-align:center" property="businessid" headerClass="sortable" sortable="true" />
						<display:column title="业务中文名称" style="width:18%;text-align:center" property="businessname" headerClass="sortable" sortable="true" />
						<display:column title="业务费率(%)" style="width:10%;text-align:center" property="rate" headerClass="sortable" sortable="true" />
						...
						<shiro:hasPermission name="posp:tabBusinessRates:edit">	
							<a href="#" onclick="editClicks('<c:out value="${displayTable.businessid}"/>','findItem')">编辑</a>
						</shiro:hasPermission>
						</display:column>
					</display:table>
					<!-- 用Diaplay Tag来显示 -->		
