...
<head>
	...
	function editClicks(params,method)
	{
		...
		document.forms[0].loginId.value=params;
		document.forms[0].submit();
...
<body onload="init();">
	...
	<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" class="table1_border">
		...
		<tr>
			<td align="center">
				<html:form action="/merchantUser" method="post">
					...
					<!-- 用Diaplay Tag来显示 -->
					<display:table name="pageInfoResult.resultItems" partialList="true" size="pageInfoResult.totalCount" pagesize="8" class="dpTable" id="displayTable" requestURI="/merchantUser.do">
						<display:column title="选择" style="width:5%;text-align:center">
							
						<input type="checkbox" name="checkItem" value="<c:out value="${displayTable.loginId}"/>">
						
						</display:column>
						<display:column title="登录名" style="width:15%;text-align:center" property="loginId" headerClass="sortable" sortable="true" />
						<display:column title="显示名" style="width:18%;text-align:center" property="loginName" headerClass="sortable" sortable="true" />
						<display:column title="状态" style="width:8%;text-align:center">
						<c:if test='${displayTable.status eq "0"}'>正常</c:if>
						</display:column>
						<display:column title="更新柜员" style="width:10%;text-align:center" property="updateOper" headerClass="sortable" sortable="true" />
						<display:column title="更新日期时间" style="width:14%;text-align:center" property="updateTime" headerClass="sortable" sortable="true" />
						<display:column title="操作" style="width:8%;text-align:center">
						<c:if test="${currentUserData.groupMap.x010208x02=='yes'}">
							<a href="#" onclick="editClicks('<c:out value="${displayTable.loginId}"/>','findItem')">编辑</a>
						</c:if>	
						</display:column>
					</display:table>
					<!-- 用Diaplay Tag来显示 -->		
					...
					<table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="table_border">
						<tr>
							...
							<td>
								<c:if test="${currentUserData.groupMap.x010208x03=='yes'}">
								...
								<c:if test="${currentUserData.groupMap.x010208x01=='yes'}">
