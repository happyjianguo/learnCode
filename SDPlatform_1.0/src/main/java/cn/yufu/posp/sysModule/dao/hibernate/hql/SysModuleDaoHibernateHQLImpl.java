...
public class SysModuleDaoHibernateHQLImpl extends OABaseDaoHibernateHQLImpl implements SysModuleDaoHibernateHQL {
	private static final Log log = LogFactory.getLog("sys");

	public SysModuleDaoHibernateHQLImpl() {
	...
	public int querySum(SysModuleModel queryModel, UserData ud) throws OAException {
		int count = 0;
		try {
			log.info("SysModuleDaoHibernateHQLImpl.querySum()开始调用：查询符合条件的数量。");
			...
			log.info("SysModuleDaoHibernateHQLImpl.querySum()结束调用：查询符合条件的数量。");
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();

			log.error("SysModuleDaoHibernateHQLImpl.querySum()调用时出现异常。");
	...
	public List queryAll(SysModuleModel queryModel, int startIndex, int maxresults, String orderField, String orderType, UserData ud) throws OAException {
		List list = null;
		try {
			log.info("SysModuleDaoHibernateHQLImpl.queryAll()开始调用：通过查询条件分页查询信息。");
			...
			log.info("SysModuleDaoHibernateHQLImpl.queryAll()结束调用：通过查询条件分页查询信息。");
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();

			log.error("SysModuleDaoHibernateHQLImpl.queryAll()通过查询条件分页查询信息，出现异常。");
			log.error(e, e.fillInStackTrace());
	...
	public void deleteItem(List newKeys, UserData ud) throws OAException {
		try {
			log.info("SysModuleDaoHibernateHQLImpl.deleteItem()调用开始：删除一条记录。");
			...
			log.info("SysModuleDaoHibernateHQLImpl.deleteItem()结束调用：删除一条记录。");
		} catch (Exception e) {
			log.error("SysModuleDaoHibernateHQLImpl.deleteItem()删除一条记录，出现异常。");
			log.error(e, e.fillInStackTrace());
			if (log.isDebugEnabled())
				e.printStackTrace();
	...
	public void createItem(SysModuleModel newModel, UserData ud) throws OAException {
		...
		try {
			log.info("SysModuleDaoHibernateHQLImpl.createItem()开始调用：保存一个信息。");

			save(newModel, ud);

			log.info("SysModuleDaoHibernateHQLImpl.createItem()结束调用：保存一个信息。");
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();

			log.error("SysModuleDaoHibernateHQLImpl.createItem()保存一个信息，出现异常。");
			log.error(e, e.fillInStackTrace());
	...
	public SysModuleModel findItem(String newKey, UserData ud) throws OAException {
		...
		try {
			log.info("SysModuleDaoHibernateHQLImpl.findItem()开始调用：显示一条记录。");
			List list = null;
			...
			log.info("SysModuleDaoHibernateHQLImpl.findItem()结束调用：显示一条记录。");
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();

			log.error("SysModuleDaoHibernateHQLImpl.findItem()显示一条记录，出现异常。");
	...
	public void saveItem(SysModuleModel newModel, UserData ud) throws OAException {
		...
		try {
			log.info("SysModuleDaoHibernateHQLImpl.saveItem()开始调用：修改一条记录。");

			update(newModel, ud);

			log.info("SysModuleDaoHibernateHQLImpl.saveItem()结束调用：修改一条记录。");
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();

			log.error("SysModuleDaoHibernateHQLImpl.saveItem()修改一条记录出现异常。");
			log.error(e.getMessage(), e.fillInStackTrace());
