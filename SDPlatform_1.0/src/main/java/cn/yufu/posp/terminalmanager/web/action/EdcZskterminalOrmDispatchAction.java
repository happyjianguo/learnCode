/**
 *包名:cn.yufu.posp.terminalmanager.web.action
 *描述:package cn.yufu.posp.terminalmanager.web.action;
 */
...
import cn.yufu.posp.terminalmanager.domain.logic.EdcZskterminalOrmLogicInterface;
...
import cn.yufu.posp.terminalmanager.web.form.EdcZskterminalOrmForm;
import cn.yufu.posp.terminalmanager.web.form.EdcZskterminalOrmUploadForm;
import net.sf.json.JSONObject;

/**
 * EdcZskterminalOrmDispatchAction.java
 * 版权所有(C) 2018 裕福控股有限公司
 * 创建:gll 
 * 时间:2018年3月2日
 * 描述:专属卡终端信息管理
 */
public class EdcZskterminalOrmDispatchAction extends OABaseDispatchAction {
	private static final Log log = LogFactory.getLog("edcTerminal");

	public EdcZskterminalOrmDispatchAction() {
	}
	/**
	 * 支付宝、微信终端管理界面
	 * 
	...
	public ActionForward page(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws OAException {
		...
		try {
			...
			log.info("edcZskterminalOrm.queryAll()开始调用：查找" + getSessionUserData(request).getUserId());
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();

			log.error("edcZskterminalOrm.queryAll()调用出现异常。");
	...
	/*** 查询终端资料设定 **/
	public org.apache.struts.action.ActionForward queryEdcZskterminalOrm(org.apache.struts.action.ActionMapping mapping, org.apache.struts.action.ActionForm form,
			javax.servlet.http.HttpServletRequest request, javax.servlet.http.HttpServletResponse response) throws OAException {
		String pathForward = "";

		try {
			log.info("EdcZskterminalOrmDispatchAction.queryEdcAlipayWeChat()开始调用：查找符合条件的记录。");

			// 得到Logic(就是service层)
			EdcZskterminalOrmLogicInterface logic = (EdcZskterminalOrmLogicInterface) getBean("edcZskterminalOrmLogic");
			...
			// 设置查询条件
			EdcZskterminalOrm queryModel = new EdcZskterminalOrm();
			EdcZskterminalOrmForm queryForm = (EdcZskterminalOrmForm) form;
			// 按商户编号查询
			...
			log.info("EdcZskterminalOrmDispatchAction.queryEdcAlipayWeChat()结束调用：查找符合条件的记录。");
		} catch (Exception e) {
			log.info("EdcZskterminalOrmDispatchAction.queryEdcAlipayWeChat()查找符合条件的记录，出现异常。");
			if (log.isDebugEnabled())
				e.printStackTrace();
			log.error(e, e.fillInStackTrace());
	...
	public org.apache.struts.action.ActionForward deleteEdcAlipayWeChat(org.apache.struts.action.ActionMapping mapping,
			...
			throws OAException {
		List<EdcZskterminalOrm> keysList = new ArrayList<EdcZskterminalOrm>();
		try {
			log.info("EdcZskterminalOrmDispatchAction.deleteEdcAlipayWeChat()开始调用：删除记录。");
			...
			for (int i = 0; i < InfoIdStrs.length; i++) {
				...
				if (tt.length == 4) {
					...
					EdcZskterminalOrm edcAlipayWeChat = new EdcZskterminalOrm();
			...
			for (EdcZskterminalOrm list : keysList) {
			...
			log.info("EdcZskterminalOrmDispatchAction.deleteEdcAlipayWeChat()结束调用：删除记录。");
		} catch (Exception e) {

			log.info("EdcZskterminalOrmDispatchAction.deleteEdcAlipayWeChat()删除记录，出现异常。");
			e.printStackTrace();
			throw new OAException(ExceptionConstants.OA_COMMON_STRUTSACTION);
		}
	...
	public org.apache.struts.action.ActionForward createEdcAlipayWeChat(org.apache.struts.action.ActionMapping mapping,
			...
			throws OAException {
		try {
			log.info("EdcZskterminalOrmDispatchAction.createEdcNewfkterminalOrm()开始调用：创建一条新的信息。");

			UserData ud = getSessionUserData(request);
			// 得到Logic
			EdcZskterminalOrmLogicInterface logic = (EdcZskterminalOrmLogicInterface) getBean("edcZskterminalOrmLogic");
			// 得到Form
			EdcZskterminalOrmForm edcAlipayWeChatForm = (EdcZskterminalOrmForm) form;
			// 新建一个Model
			EdcZskterminalOrm edcAlipayWeChat = new EdcZskterminalOrm();
			...
			log.info("EdcZskterminalOrmDispatchAction.createEdcNewfkterminalOrm()结束调用：创建一条新的信息。");

		} catch (Exception e) {
			log.info("EdcZskterminalOrmDispatchAction.createEdcNewfkterminalOrm()创建一条新的信息，出现异常。");
			if (log.isDebugEnabled())
				e.printStackTrace();
			log.error(e, e.fillInStackTrace());
			throw new OAException(ExceptionConstants.OA_COMMON_STRUTSACTION);
	...
	public org.apache.struts.action.ActionForward queryEdcAlipayWeChatByKey(org.apache.struts.action.ActionMapping mapping,
		...
		try {
			log.info("EdcZskterminalOrmDispatchAction.queryEdcAlipayWeChatByKey()开始调用：显示修改终端资料设定信息界面。");
			EdcZskterminalOrmForm newForm = (EdcZskterminalOrmForm) form;
			// System.out.println("form::" + form);
			if ("".equals(newForm.getMerchantId()) || newForm.getMerchantId() == null) {
				log.info("EdcZskterminalOrmDispatchAction.queryEdcAlipayWeChatByKey()线束调用：显示新增终端资料设定信息界面。");
			...
			EdcZskterminalOrm edcZskterminalOrm = new EdcZskterminalOrm();

			if (tt.length == 4) {
				edcZskterminalOrm.setMerchantId(tt[0].trim());
				edcZskterminalOrm.setTerminalId(tt[1].trim());
				edcZskterminalOrm.setModuleId(tt[2].trim());
			}
			...
			EdcZskterminalOrmLogicInterface logic = (EdcZskterminalOrmLogicInterface) getBean("edcZskterminalOrmLogic");
			// 得到Form
			EdcZskterminalOrmForm edcZskterminalOrmForm = (EdcZskterminalOrmForm) form;

			map = logic.findItemByKey(edcZskterminalOrm, ud);

			EdcZskterminalOrm model = (EdcZskterminalOrm) map.get("Infolist");

			BeanUtils.copyProperties(edcZskterminalOrmForm, model);

			log.info("EdcZskterminalOrmDispatchAction.queryEdcTerminalByKey()结束调用：显示修改终端资料设定信息界面。");

		} catch (Exception e) {

			log.info("EdcZskterminalOrmDispatchAction.queryEdcTerminalByKey()显示修改终端资料设定信息界面，出现异常。");
			if (log.isDebugEnabled())
				e.printStackTrace();
	...
	public org.apache.struts.action.ActionForward checkMerTerMod(org.apache.struts.action.ActionMapping mapping, org.apache.struts.action.ActionForm form,
		...
		try {
			log.info("EdcZskterminalOrmDispatchAction.checkMerTerMod()开始调用：验证商户编号、终端号 和模块ID三者的唯一性。");
			...
			//封装
			EdcZskterminalOrm edcZskterminalOrm = new EdcZskterminalOrm();
			edcZskterminalOrm.setMerchantId(merchantId);
			edcZskterminalOrm.setTerminalId(terminalId);
			edcZskterminalOrm.setModuleId(moduleId);
	...
	public org.apache.struts.action.ActionForward saveEdcAlipayWeChat(org.apache.struts.action.ActionMapping mapping, org.apache.struts.action.ActionForm form,
		...
		try {
			log.info("EdcZskterminalOrmDispatchAction.saveEdcNewfkterminalOrm()开始调用：修改终端资料设定。");
			...
			log.info("EdcZskterminalOrmDispatchAction.saveEdcNewfkterminalOrm()结束调用：修改终端资料设定。");
		} catch (Exception e) {
			log.info("EdcZskterminalOrmDispatchAction.saveEdcNewfkterminalOrm()修改终端资料设定，出现异常。");
			if (log.isDebugEnabled())
				e.printStackTrace();
			log.error(e, e.fillInStackTrace());
			throw new OAException(ExceptionConstants.OA_COMMON_STRUTSACTION);
	...
	public org.apache.struts.action.ActionForward downloadTemplate(org.apache.struts.action.ActionMapping mapping, org.apache.struts.action.ActionForm form,
		...
		try {
			...
			cell.setCellValue("请按模板整理数据！商户编号、商户终端编号、银行商户号、银行终端号、模块ID、批次号、工作密钥密文MAC_KEY必须均存在！");
			...
			cell = null;
			String[] head1 = {"商户编号","商户终端编号","银行商户号","银行终端号","模块ID","系统流水号","银行流水号","批次号","工作密钥密文MAC_KEY"};
			...
			POIUtils.createTextCell(book, row, (short) 3, "00000002");
			POIUtils.createTextCell(book, row, (short) 4, "77");
			POIUtils.createTextCell(book, row, (short) 5, "1");
			POIUtils.createTextCell(book, row, (short) 6, "1");
			POIUtils.createTextCell(book, row, (short) 7, "1");
			POIUtils.createTextCell(book, row, (short) 8, "11111111111111111111111111111111");
			// 对文件名进行编码
			String headName = "专属卡终端导入模板";
			...
			if (log.isDebugEnabled())
				e.printStackTrace();
			log.error("EdcZskterminalOrmDispatchAction.queryDetail()调用出现异常。");
			log.error(e, e.fillInStackTrace());
			throw new OAException(ExceptionConstants.OA_COMMON_STRUTSACTION);
		}
		return null;
	...
	public org.apache.struts.action.ActionForward fileUpload(org.apache.struts.action.ActionMapping mapping, org.apache.struts.action.ActionForm form,
		...
		try {
			...
			EdcZskterminalOrmUploadForm uf = (EdcZskterminalOrmUploadForm) form;
			...
			if (byteFile.length > maxFileSize) {
				...
				for (int index = 2; index < sheet.getLastRowNum() + 1; index++) {
					...
					//银行终端号
					String bankTerminalId = POIUtils.getStringFromExcelCell(row.getCell(3));
					if (StringUtil.isNull(bankTerminalId)) {
						break;
					}
					edcAlipayWeChat.setBankTerminalId(bankTerminalId);
					
					String moduleId = POIUtils.getStringFromExcelCell(row.getCell(4));
					if (StringUtil.isNull(moduleId)) {
						break;
					}
					edcAlipayWeChat.setModuleId(moduleId);
					
					String sysTrace = POIUtils.getStringFromExcelCell(row.getCell(5));
					if (StringUtil.isNull(sysTrace)) {
						break;
					}
					edcAlipayWeChat.setSysTrace(sysTrace);
					
					String bankTrace = POIUtils.getStringFromExcelCell(row.getCell(6));
					if (StringUtil.isNull(bankTrace)) {
						break;
					}
					edcAlipayWeChat.setBankTrace(bankTrace);
					
					String batchNo = POIUtils.getStringFromExcelCell(row.getCell(7));
					if (StringUtil.isNull(batchNo)) {
						break;
					}
					edcAlipayWeChat.setBatchNo(batchNo);
					
					String makTmk = POIUtils.getStringFromExcelCell(row.getCell(8));
					...
					edcAlipayWeChat.setKeyIndex("01");//密钥索引
