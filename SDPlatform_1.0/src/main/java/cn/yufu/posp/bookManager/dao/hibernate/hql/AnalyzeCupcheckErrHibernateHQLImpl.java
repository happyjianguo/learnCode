...
public class AnalyzeCupcheckErrHibernateHQLImpl extends OABaseDaoHibernateHQLImpl implements AnalyzeCupcheckErrHibernateHQL {
	...
	public List queryAll(AnalyzeCupcheckErr queryModel, int startIndex, int pageSize, String orderField, String orderType, UserData ud) throws OAException {
		List list = null;
		try {
			log.info("AnalyzeCupcheckErrHibernateHQLImpl.queryAll()开始调用：通过查询条件分页查询信息。");

			DetachedCriteria dcr = DetachedCriteria.forClass(AnalyzeCupcheckErr.class);
			...
			log.info("AnalyzeCupcheckErrHibernateHQLImpl.queryAll()结束调用：通过查询条件分页查询信息。");
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();
			log.error("AnalyzeCupcheckErrHibernateHQLImpl.queryAll()通过查询条件分页查询信息，出现异常。");
	...
	public int queryCount(AnalyzeCupcheckErr queryModel, UserData ud) throws OAException {
		...
		try {
			...
			StringBuffer bufferHql = new StringBuffer("select count(*) from AnalyzeCupcheckErr o where 1=1");
	...
	public AnalyzeCupcheckErr queryDetail(String id, UserData ud) throws OAException {
		AnalyzeCupcheckErr model = null;
		try {
			log.info("AnalyzeCupcheckErrHibernateHQLImpl.queryDetail()开始调用：显示一条记录。");
			List list = null;
			Session session = getSession();
			// 基本信息
			Criteria cr = session.createCriteria(AnalyzeCupcheckErr.class);
			cr.add(Restrictions.eq("id", id));
			list = findByCriteria(cr, ud);
			if (list.size() > 0)
				model = (AnalyzeCupcheckErr) list.get(0);
			log.info("AnalyzeCupcheckErrHibernateHQLImpl.queryDetail()结束调用：显示一条记录。");
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();
			log.error("AnalyzeCupcheckErrHibernateHQLImpl.queryDetail()显示一条记录，出现异常。");
