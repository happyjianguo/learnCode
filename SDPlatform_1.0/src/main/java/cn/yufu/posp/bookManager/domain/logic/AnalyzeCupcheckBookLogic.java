...
public class AnalyzeCupcheckBookLogic extends BaseLogic implements AnalyzeCupcheckBookLogicInterface {
	...
	public PageInfoModel queryAll(AnalyzeCupcheckBook queryModel, PageInfoModel pageInfo, UserData ud) throws OAException {
		...
		try {
			log.info("AnalyzeCupcheckBookLogic.queryAll()开始调用：分页查询。" + ud.getUserId());
			...
			log.info("AnalyzeCupcheckBookLogic.queryAll()结束调用：分页查询。" + ud.getUserId());

		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();
			log.error("AnalyzeCupcheckBookLogic.queryAll()调用时出现异常。");
	...
	public AnalyzeCupcheckBook queryDetail(String id, UserData ud) throws OAException {
		...
		try {
			log.info("AnalyzeCupcheckBookLogic.queryDetail()开始调用：显示一条记录。" + ud.getUserId());
			...
			log.info("AnalyzeCupcheckBookLogic.queryDetail()结束调用：显示一条记录。" + ud.getUserId());
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();
