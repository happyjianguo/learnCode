...
public class AnalyzeCupcheckStaticsLogic extends BaseLogic implements AnalyzeCupcheckStaticsLogicInterface {
	...
	public PageInfoModel queryAll(AnalyzeCupcheckStatics queryModel, PageInfoModel pageInfo, UserData ud) throws OAException {
		...
		try {
			log.info("AnalyzeCupcheckStaticsLogic.queryAll()开始调用：分页查询。" + ud.getUserId());
			...
			log.info("AnalyzeCupcheckStaticsLogic.queryAll()结束调用：分页查询。" + ud.getUserId());

		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();
			log.error("AnalyzeCupcheckStaticsLogic.queryAll()调用时出现异常。");
	...
	public AnalyzeCupcheckStatics queryDetail(String id, UserData ud) throws OAException {
		...
		try {
			log.info("AnalyzeCupcheckStaticsLogic.queryDetail()开始调用：显示一条记录。" + ud.getUserId());
			...
			log.info("AnalyzeCupcheckStaticsLogic.queryDetail()结束调用：显示一条记录。" + ud.getUserId());
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();
