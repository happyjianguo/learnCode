...
public class AnalyzeMerclearBookAction extends OABaseDispatchAction {
	...
	public ActionForward queryAll(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws OAException {
		String pathForward = "";
		try {
			log.info("AnalyzeMerclearBookAction.queryAll()开始调用：查找" + getSessionUserData(request).getUserId());
			...
			if (log.isDebugEnabled())
				e.printStackTrace();
			log.error("AnalyzeMerclearBookAction.queryAll()调用出现异常。");
	...
	public org.apache.struts.action.ActionForward queryDetail(org.apache.struts.action.ActionMapping mapping, org.apache.struts.action.ActionForm form,
		...
		try {
			log.info("AnalyzeMerclearBookAction.queryDetail()开始调用：显示修改的界面" + getSessionUserData(request).getUserId());
			...
			log.info("AnalyzeMerclearBookAction.queryDetail()结束调用：显示修改的界面" + getSessionUserData(request).getUserId());
		} catch (Exception e) {
			if (log.isDebugEnabled())
				e.printStackTrace();
			log.error("AnalyzeMerclearBookAction.queryDetail()显示修改办公用品物品类别界面,调用出现异常。");
			log.error(e, e.fillInStackTrace());
			throw new OAException(e.getMessage());
		}
