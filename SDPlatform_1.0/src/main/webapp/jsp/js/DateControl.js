...
function DateCheck(stTime,enTime,str){
	var el = document.getElementById(stTime);
	var el1 = document.getElementById(enTime);
	var str
	if (isTime(el.value)==false) {
		alert(str+"日期：开始时间输入不合法(且不能为空),请重新选择!");
		el.focus();
		return   false;
	}
	if (isTime(el1.value)==false) {
		alert(str+"日期：结束时间输入不合法(且不能为空),请重新选择!");
		el1.focus();
		return   false;
	}
	...
	{
		alert(str+"日期：开始时间必须小于(等于)结束时间,请重新选择!");
		el.focus();
		return   false;
	}	
	return true;
...
function CalendarInit()				///Create panel
{
	...
	htmloDayTable+="<div align='center' style='padding:3px'><font style='font-size:13px;border:1px solid #BCD0DE;align:center;width:100%'>当前时间：<span id=CurTime>"+GetCurTime()+"</span></font></div>";
	/*新增当前时间_end*/
...
function getDateString(oInputSrc,oInstance)
{
	if(oInputSrc&&oInstance)
	{
		...
		document.getElementById("CurTime").innerText= GetCurTime();
	}
}
function CalendarCellSetCss(sMode,oCell)			/// Set Cell Css
{
	...
	if(sMode)
	{
		oCell.style.border="1px solid #93BBF6";
