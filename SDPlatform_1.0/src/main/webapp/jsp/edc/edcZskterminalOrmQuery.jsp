...
<head>
	...
	function addClick(){
		document.forms[0]._id.value="";
		document.forms[0].method.value="queryEdcAlipayWeChatByKey";
...
<body onload="init();">
	<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" class="table1_border">
		...
		<tr>
			<td align="center">
				<html:form action="/edcZskterminalOrm" method="post">
					<input type="hidden" name="method" value="queryEdcZskterminalOrm" />
					<!----------以下Table为查询form-------->
					<table width="96%" border="0" align="center" cellpadding="0" cellspacing="1" class="table2_border">
						...
						<tr>
							<td colspan="6" class="table2_btn">
								<input name="imageField" type="image" src="<fmt:message key='CommonImagePath' />btnQuery.gif" alt="查询" width="65" height="20" border="0" onclick="return searchClick('queryEdcZskterminalOrm');">
					...
					<!-- 用Diaplay Tag来显示 -->
					<display:table name="pageInfoResult.resultItems" partialList="true" size="pageInfoResult.totalCount" pagesize="8" class="dpTable" id="displayTable" requestURI="/edcZskterminalOrm.do">
						...
						<display:column title="专属卡商户号" style="width:13%;text-align:center" property="bankMerchantId" headerClass="sortable" sortable="true" />
						<display:column title="专属卡终端号" style="width:15%;text-align:center" property="bankTerminalId" headerClass="sortable" sortable="true" />
						<display:column title="批次号" style="width:10%;text-align:center" property="batchNo" headerClass="sortable" sortable="true" />
						<display:column title="系统流水号" style="width:10%;text-align:center" property="sysTrace" headerClass="sortable" sortable="true" />
                        ...
                        <shiro:hasPermission name="posp:alipaywechat:view">
						   <a href="#" onclick="javascript:editClicks('<c:out value="${displayTable.merchantId}#${displayTable.terminalId}#${displayTable.moduleId}#end"/>','queryEdcAlipayWeChatByKey')">查看</a>
                        </shiro:hasPermission> 
						</display:column>
						...
						<tr>
							...
							<td>
                                <shiro:hasPermission name="posp:edczskterminalorm:add">
									<a href="javascript:deleteClick('selectItems','checkItem','deleteEdcAlipayWeChat');"><img border=0 name="imageField33" src="<fmt:message key='CommonImagePath' />btnDelete.gif" alt="删除" width="65" height="20" border="0"></a>
                                </shiro:hasPermission>
                                <shiro:hasPermission name="posp:edczskterminalorm:add">
	...
	<shiro:hasPermission name="posp:edczskterminalorm:add">
		<center>			
			<form id='formShowData' name="formShowData" enctype='multipart/form-data' method='post' action="edcZskterminalOrmUpload.do" >
				<input type="hidden" name="method" value="fileUpload" />
				<input type="file" name='upload' id='upload' accept="application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" /><input type="button" value="导入" onclick="fileUpload();" />
				<a href="javascript:;" onclick="downloadTemplate();" style="color: red;">下载导入模板</a>
			</form>
		</center>
	</shiro:hasPermission>
</body>
