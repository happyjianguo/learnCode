...
<head>
	...
	function showClicks(id,method)
	{
		document.forms[0].method.value=method;
		document.forms[0]._id.value=id;
		document.forms[0].showFlag.value="show";
		document.forms[0].submit();
...
<shiro:lacksPermission name="posp:tmkmasterkey:view">
    <div class="permission_error">用户[${fns:getUserName()}]没有该权限<br/></div>
</shiro:lacksPermission>
<shiro:hasPermission name="posp:tmkmasterkey:view">
<body onload="init();">
				...
				<html:form action="/TMKMasterKEY" method="post">
					...
					<display:table name="pageInfoResult.resultItems" partialList="true" size="pageInfoResult.totalCount" pagesize="8" class="dpTable" id="displayTable" requestURI="/TMKMasterKEY.do">
						...
						<shiro:hasPermission name="posp:tmkmasterkey:edit">                        
                         <c:if test="${displayTable.settleFlag!='0'}">
							   <a href="#" onclick="javascript:showClicks('<c:out value="${displayTable.merchantId}#${displayTable.terminalId}#end"/>','findItem')">查看</a>
	                        </c:if>    
	                        <c:if test="${displayTable.settleFlag=='0'}">
							   <a href="#" onclick="javascript:editClicks('<c:out value="${displayTable.merchantId}#${displayTable.terminalId}#end"/>','findItem')">生成</a>
	                        </c:if> 
	                        <c:if test="${displayTable.settleFlag!='0'}">
							   <a href="#" onclick="javascript:editClicks('<c:out value="${displayTable.merchantId}#${displayTable.terminalId}#end"/>','findItem')">更新</a>
	                        </c:if> 
                         </shiro:hasPermission> 
						</display:column>
					...
					<table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="table_border">
						<tr>
							...
							<td>
                                <shiro:hasPermission name="posp:tmkmasterkey:batch">
