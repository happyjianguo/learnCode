 ...
 * @version 1.0
 */
public class SixBFlowLogDao {
    private static final Logger logger = Logger.getLogger(SixBFlowLogDao.class);
    private String sql = "";
    ...
    public List<SixBFlowLogBean> getBFlowLogList(String role_no, String dept_no_node,PageBean pageBean) {
    	java.text.SimpleDateFormat sdf =new java.text.SimpleDateFormat("yyyyMMdd");
    	String nowdate=sdf.format(new Date());
    	List<SixBFlowLogBean> list = new ArrayList<SixBFlowLogBean>();
    	sql="select b.id,b.batchflag,b.execdate,b.issucess,b.execinfo,b.addtime,b.srcstartflag,b.srcendflag,b.descstartflag,b.execflag,"
    			...
    			+ "from monbatchinfo c,MonSixBatchflwlog b "
		...
		try {
			...
			if (bFlowLogVector != null && !bFlowLogVector.isEmpty()) {
				for (int i = 0; i < bFlowLogVector.size(); i++) {
					SixBFlowLogBean bmangerBean = new SixBFlowLogBean((HashMap) bFlowLogVector.get(i));
    ...
    public List<SixBFlowLogBean> queryBFlowLogList(String role_no, String dept_no_node,PageBean pageBean,SixBFlowLogBean bean) {
    	List<SixBFlowLogBean> list = new ArrayList<SixBFlowLogBean>();
     ...
     * @return
     */
    public List<SixBFlowLogBean> getBinList(String isMCBin) {    	
    	List<SixBFlowLogBean> list = new ArrayList<SixBFlowLogBean>();
    	if("1".equals(isMCBin)){
    ...
    public int getCount(String roleno,String dept_no_node) {
    	...
    	java.text.SimpleDateFormat sdf =new java.text.SimpleDateFormat("yyyyMMdd");
    	String nowdate=sdf.format(new Date());
    	sql="select count(*) numbers from monbatchinfo b,MonSixBatchflwlog f where f.batchflag = b.batchflag  and substr(f.execdate,1,8)='"+nowdate+"'   and datasource=0";
    ...
    public int getCount(String roleno,String dept_no_node,SixBFlowLogBean bean) {
    	int count=0;
    	
    	sql="select count(*) numbers from monbatchinfo b,MonSixBatchflwlog f where f.batchflag = b.batchflag  ";
    ...
    public SixBFlowLogBean getBFlowLogInfo(String id) {
        sql = "select t.*,b.dept_no,b.batchname,b.batchfile,b.batchflagfile,b.demo,b.backuppath,b.logfilepath,srcstartflagname,srcendflagname,descstartflagname,descendflagname,b.socketflag from monbatchinfo b,MonSixBatchflwlog t where t.batchflag = b.batchflag  and t.id = ? ";
        List paramList = new ArrayList();
        paramList.add(id);
        SixBFlowLogBean flowlogBean=new SixBFlowLogBean();
        try {
            Vector bflowlogVector = DbExec.execQueryKeep(sql,paramList);
            if (bflowlogVector != null && !bflowlogVector.isEmpty()) {
            	flowlogBean = new SixBFlowLogBean((HashMap) bflowlogVector.get(0));                    
              }
     ...
     * */
    public int getSixBFlowLogInfoStatus(String panflagno,String batchflag,SixBFlowLogBean sixflowlogBean) {
        sql = "select t.* from MonSixBatchflwlog t where  t.batchflag = ? and panflagno=?";
        ...
        try {
            Vector bflowlogVector = DbExec.execQueryKeep(sql,paramList);
            if (bflowlogVector != null && !bflowlogVector.isEmpty()) {
            	sixflowlogBean = new SixBFlowLogBean((HashMap) bflowlogVector.get(0)); 
            	if("0".equals(sixflowlogBean.getIssucess())){
            		result=0; //执行成功
            	}else if("2".equals(sixflowlogBean.getIssucess())){
     ...
     * @return 0 执行成功 -1 执行状态失败  -2 记录为空 2 是初始化状态 
     * */
    public int getBFlowLogInfoStatusOfNC(String panflagno,String batchflag,SixBFlowLogBean sixflowlogBean) {
        ...
        try {
            ...
            if (bflowlogVector != null && !bflowlogVector.isEmpty()) {
            	sixflowlogBean = new SixBFlowLogBean((HashMap) bflowlogVector.get(0)); 
            	if("01".equals(sixflowlogBean.getIssucess())){
            		result=0; //执行成功
            	}else if("00".equals(sixflowlogBean.getIssucess())){
            		result=2; //执行成功 发卡程序仍在进行
            	}else{
            		result=-1; //执行失败
            	}
     ...
     * @return
     */
    public SixBFlowLogBean getSameNameInfo(String pan,String panflagno) {
     ...
     * @return
     */
    public List<SixBFlowLogBean> querySameNameList(PageBean pageBean,String pan,String stat) {
    	List<SixBFlowLogBean> list = new ArrayList<SixBFlowLogBean>();
     ...
     * @return 0 执行成功 -1 执行状态失败  -2 记录为空 2 是初始化状态 
     * */
    public int getMakeCardStat(String panflagno,SixBFlowLogBean flowlogBean) {
