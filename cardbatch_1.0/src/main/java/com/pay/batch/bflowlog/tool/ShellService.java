...
//							    	   t.start();
							    	   Thread.sleep(5);
							    	   util.setValue("msginfo","脚本正在执行请耐心等待");	
							       }else  if(result==0){
//							    	   Thread t=new Thread(new ThreadUtilSet(util,"msginfo","脚本执行成功,请查看流水记录"));
//							    	   t.start();
							    	   Thread.sleep(5);
								     util.setValue("msginfo", "脚本执行成功,请查看流水记录");								      
							       }else  if(result==2){
//							    	   Thread t=new Thread(new ThreadUtilSet(util,"msginfo","脚本执行成功,请查看流水记录"));
//							    	   t.start();
							    	   Thread.sleep(5);
								     util.setValue("msginfo", "脚本已初始化正在执行请耐心等待"+count);								      
...
//							    	   t.start();
//							    	   Thread.sleep(5);
							    	  util.setValue("msginfo", "记录为空脚本正在执行请耐心等待"); 
							       }							      
							      count++;
								   if(count>10)
								   {
									   result=-1;
									   util.setValue("msginfo", "脚本执行失败请联系管理员");	
									   break;
								   }
								  Thread.sleep(1000);
							  }while(result!=0);		         
