...
dTree.prototype.node = function(node, nodeId) {
	...
	if(node.pid == this.root.id){
		...
		checkboxSyntax = "<input type='checkbox' desc='" + node.cshow + "' name='" + node.cname + "' id='" + node.cname + "_" + node.id + "_" + node.pid + "' value='" + node.cvalue + "' onClick='javascript: " + this.obj + ".checkNode(\"" + node.id+"\","+node.pid+","+node._hc + ",this.checked);' ";
		if(node.cchecked)
...
dTree.prototype.isHaveBNode = function(id,pid) {	
	...
	for (var n=0; n<this.aNodes.length; n++) {
		if (this.aNodes[n].pid!=-1&&this.aNodes[n].id!=id&&this.aNodes[n].pid == pid) {			
			if(eval("document.getElementById(\""+ this.aNodes[n].cname + "_" + this.aNodes[n].id + "_" + this.aNodes[n].pid + "\").checked"))
				isChecked = true;			
		}
	}
...
dTree.prototype.checkPNodeRecursion = function(pid,ischecked) {	
	for (var n=0; n<this.aNodes.length; n++) {
		if (this.aNodes[n].pid!=-1&&this.aNodes[n].id == pid) {			
			eval("document.getElementById(\""+ this.aNodes[n].cname + "_" + this.aNodes[n].id + "_" + this.aNodes[n].pid + "\").checked = " + ischecked);
