...
<script language="javascript">
	function addConfirm(){
		...
		sysParameterForm.action = "<%=path%>/sysParameter.do?method=preAddSysParameter";
		sysParameterForm.submit();
	}
	
	function query(){
		var sysParameterForm = document.all("sysParameterForm");
		sysParameterForm.action = "<%=path%>/sysParameter.do?method=getSysParameterList";
		sysParameterForm.submit();
	}		
	function deleteClick(id){
		if(confirm("确定要删除编号为"+id+"的参数信息？")){
			...
			var sendUrl="<%=path%>/sysParameter.do?method=sysParameterHasChild&id="+id;
			startAjax(sendUrl);
		}
	}
	        ...
	        if(xmlHttp.status == 200) { 
	           ...
	           if(xmlHttp.responseText==0){
	        	   var id=document.getElementById("id").val;
	        	    var sysParameterForm = document.all("sysParameterForm");
		   			sysParameterForm.action = "<%=path%>/sysParameter.do?method=delSysParameter&id="+id;
		   			sysParameterForm.submit();	
...
<body>
	<html:form action="/sysParameter?method=getSysParameterList" method="post" styleId="sysParameterForm">
			...
			<tr>
				<td align="center" valign="top" height="87%">
					...
					<table cellpadding="0" border="0" cellspacing="0" width="100%" style="padding: 0px;" align="left">
						...
						<tr>
						<td colspan="10">
							<display:table name="SysParameterList" partialList="true" size="pageBean.totalRecords" pagesize="10" style="width:100%" class="dpTable" cellpadding="0" cellspacing="0" id="displayTable" requestURI="/sysParameter.do">
								...
								<display:column title="查看" style="text-align:center" href="/sysParameter.do?method=preQuerySysParameter" paramId="id" paramProperty="param_id">
									<img border="0" src="<%=path%>/image1/border/query.png" />
								</display:column>
								<display:column title="修改" style="text-align:center" href="/sysParameter.do?method=preModSysParameter" paramId="id" paramProperty="param_id">
									修改
								</display:column>
