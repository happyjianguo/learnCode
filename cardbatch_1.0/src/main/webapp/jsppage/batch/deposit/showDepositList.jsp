...
<script language="javascript">
	function query(){
		...
		if(addDT_startdate!='' && addDT_enddate!='' && (getDate(addDT_startdate)-getDate(addDT_enddate)>0)){
		    alert("结束时间不能小于开始时间");
		    window.document.forms['depositForm'].elements['startdate'].focus() ;
		    return false;
		} 
		...
		var depositForm = document.all("depositForm");
		depositForm.action = "<%=path%>/deposit.do?method=getDepositList";
		depositForm.submit();
	}
	function exportExcel(){	
		var depositForm = document.all("depositForm");
		depositForm.action = "<%=path%>/deposit.do?method=exportExcel";
		depositForm.submit();			
		//window.open('<%=path%>/deposit.do?method=exportExcel');
	...
	{
		var url = "<%=path%>/deposit.do?method=showDepositInfo&id="+id+"&random=" + Math.random();
	...
	{		
		document.getElementById("batchbutton").disabled=true;
	 	...
	 	if(flag)
	 	{
	 		...
	 		{	
	 			depositForm.action = "<%=path%>/deposit.do?method=createBatchCheck&msgtype=0028";
	 			depositForm.submit();
	 		}else{
...
<shiro:lacksPermission name="cardbatch:deposit:view">
    <div class="permission_error">用户[${fns:getUserName()}]没有该权限<br/></div>
</shiro:lacksPermission>
<shiro:hasPermission name="cardbatch:deposit:view">
<body bottommargin="0" leftmargin="0" topmargin="0" rightmargin="0">
	<html:form action="/deposit.do?method=getDepositList" method="post"
		styleId="depositForm">
				...
				<td align="center" valign="top" height="87%">
					...
					<table cellpadding="0" border="0" cellspacing="0" width="100%"
						...
						<tr class="serch">
							...
							<td style="white-space: nowrap" align="left" >
								<html:select property="cashin_type">
									<html:option value="">－请选择－</html:option>								
									<logic:present name="cashin_typeList">
										<html:optionsCollection name="cashin_typeList"
											label="param_name" value="param_value" />
									</logic:present>
								</html:select>
							</td>
						...
						<td colspan="13">
							<display:table name="openCardList" partialList="true" size="pageBean.totalRecords" pagesize="10" style="width:100%" class="dpTable" cellpadding="0" cellspacing="0" id="displayTable" requestURI="/deposit.do">
								...
								<shiro:hasPermission name="cardbatch:deposit:edit">
