...
//		</c:if>	
	...
	function query(){
		...
		if(nums > 30){
			...
			var tlogForm = document.all("tlogForm");
			tlogForm.action = "<%=path%>/tlog.do?method=selectqueryTLogListBak";
			<%-- tlogForm.action = "<%=path%>/tlog.do?method=queryTLogListBak"; --%>
			tlogForm.submit();
		}
	}
	//data转换
...
<shiro:lacksPermission name="cardbatch:tlog:view">
    <div class="permission_error">用户[${fns:getUserName()}]没有该权限<br/></div>
</shiro:lacksPermission>
<shiro:hasPermission name="cardbatch:tlog:view">
<body>
	<html:form action="/tlog.do?method=queryTLogListBak" method="post" styleId="tlogForm">
