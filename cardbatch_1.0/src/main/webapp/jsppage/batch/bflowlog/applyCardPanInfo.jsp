...
function commit(){
		...
		var countFlagEqZero   =   document.sixbflowlogForm.countFlagEqZero.value;
...
// 		var binvalue = obj.options[index].value;
		...
		var j = parseInt(countFlagEqZero);
		if(flagnum > j){
			alert("发卡数不能超过未制卡总数");			
		 	return false;
		}
		/* if(flagnum > countFlagEqZero){
           	alert("发卡数不能超过未制卡总数");			
		 	return false;
		} */
		if(trim(flagnumss) == ""){
...
<center>
	<body bottommargin="0" leftmargin="0" topmargin="0" rightmargin="0">
		<html:form styleId="sixbflowlogForm" action="/sixbflowlog.do?method=ApplyCardPanInfo" method="post" >
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%" height="99%" >
				<tr>
					...
					<tr>
						...
						<td   height="28" style=" background:url(<%=path%>/image1/Navigation_bar/middle.gif) repeat-x;"  > &nbsp;&nbsp;当前位置：批量管理 &gt; 特制批量发卡管理&gt; ETC发卡 </td>
						...
						<table border="0" cellpadding="0" cellspacing="0" width="700"  class = "box1">							
							...
							<tr>
								...
								<td align="left" class="box2">
									...
									(可发卡张数:${sixbflowlogForm.countFlagEqZero})
									<html:hidden property="countFlagEqZero" value="${sixbflowlogForm.countFlagEqZero}"/>									
								</td>
								<td align="left" class="box2" width="40%">
