...
<script language="javascript">
	...
	function query(){
		var sixbflowlogForm = document.all("sixbflowlogForm");
		sixbflowlogForm.action = "<%=path%>/sixbflowlog.do?method=preShowSixPanFlowLogList";
		sixbflowlogForm.submit();
	}

	function preQueryCrdformatMap(id)
	{
		var url =  "<%=path%>/sixbflowlog.do?method=showSixPanFlowLogInfo&id="+id+"&random=" + Math.random();		
	...
	{
		var url =  "<%=path%>/sixbflowlog.do?method=preAddSixPanInfo&random=" + Math.random();
		var iWidth = 1046; //弹出窗口的宽度;
		var iHeight = 400; //弹出窗口的高度;
		var iTop = (window.screen.availHeight - 30 - iHeight) / 2; //获得窗口的垂直位置;
		var iLeft = (window.screen.availWidth - 10 - iWidth) / 2; //获得窗口的水平位置;
	...
	function applycardpaninfo(id)
	{
		var url =  "<%=path%>/sixbflowlog.do?method=preApplyCardPanInfo&random=" + Math.random();
		var iWidth = 1046; //弹出窗口的宽度;
		var iHeight = 400; //弹出窗口的高度;
		var iTop = (window.screen.availHeight - 30 - iHeight) / 2; //获得窗口的垂直位置;
		var iLeft = (window.screen.availWidth - 10 - iWidth) / 2; //获得窗口的水平位置;
...
<shiro:lacksPermission name="housekeep:sixkaika:view">
    <div class="permission_error">用户[${fns:getUserName()}]没有该权限<br/></div>
</shiro:lacksPermission>
<shiro:hasPermission name="housekeep:sixkaika:view">

<body bottommargin="0" leftmargin="0" topmargin="0" rightmargin="0">
	<html:form action="/sixbflowlog.do?method=getSixBFlowLogList"
						
		method="post" styleId="sixbflowlogForm">
						...
						<tr  class="serch">
						 <shiro:hasPermission name="housekeep:kaika:faka">
							<td style="white-space: nowrap" align="right">
								<input class="button" type="button" value="万科发卡"
									onClick="addpaninfo()">
							</td>
							<td style="white-space: nowrap" align="right">
								<input class="button" type="button" value="ETC发卡"
									onClick="applycardpaninfo()">
							</td>
							</shiro:hasPermission>
							<td style="white-space: nowrap" align="right">
								受理号：
