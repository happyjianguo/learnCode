...
<script type="text/javascript">
	...
	function commit() {
		...
		/* if (!checkelement("merchantForm", "address", "商户联系地址")) {
			return false;
		} */
...
// 	   }
	 ...
	 //校验商户状态是否销户
		var mrchstat=document.getElementsByName("mrchstat")[0].value;
		if(mrchstat ==null||mrchstat ==""){
			alert("请选择商户状态");
			document.forms["mrchstat"].elements["mrchstat"].focus();
			return false;
		}else{
			if(mrchstat =="3"){
				//alert("提示：请确认，商户状态为销户后将无法进行修改");
				var ret = window.confirm("提示：请确认，商户状态为销户后将无法进行修改?");
				//当点击确定时 返回 true 
				if(ret){
				    //do something 点确定
				}else{
...
<body>
	<html:form styleId="merchantForm" action="/merchant?method=modMerchant"
		enctype="multipart/form-data" method="post">
		<html:hidden property="id" />
		<html:hidden property="merchant_id" />
		<html:hidden property="inst_id" />
		<html:hidden property="mrchno" />
		<html:hidden property="bus_lic_pic" />
		<html:hidden property="tax_id_pic" />
		<html:hidden property="org_id_pic" />
					...
					<table border="0" cellpadding="0" cellspacing="0" width="100%">
						<tr>
							...
							<td align="left" class="box2">
								<html:text property="mrchno" maxlength="15" disabled="true"></html:text>
						...
						<tr>
							...
							<td align="left" class="box2">
								<html:text property="merchant_x_reg_amt" onchange="clearNoNum(this);" maxlength="18"></html:text>
								<font color="red">*</font>
								万元  —— 币种
