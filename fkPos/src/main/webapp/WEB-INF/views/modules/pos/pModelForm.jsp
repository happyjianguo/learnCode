...
<head>
	...
	<script type="text/javascript">
		...
		var checkModelId = function() {
			...
			if(modelId!=""){
				$.ajax({
					url : '${pageContext.request.contextPath}/pos/pModel/checkModelId',
					type : "post",
					data : {
						modelId : modelId
					},
...
<body>
	...
	<form:form id="inputForm" modelAttribute="pModel" action="${pageContext.request.contextPath}/pos/pModel/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<c:if test="${not empty pModel.modelId}"> 
					<form:input path="modelId" htmlEscape="false" maxlength="64" class="input-xlarge required" readonly="true"/>
				</c:if> 
				<c:if test="${empty pModel.modelId}"> 
					<form:input path="modelId" htmlEscape="false" maxlength="64" class="input-xlarge required" onblur="checkModelId();"/>
				</c:if>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="modelStatus" class="input-xlarge required">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<c:if test="${empty pModel.modelId}">
					<form:select path="factoryId" class="input-xlarge required">
						<form:option value="" label="--请选择--"/>
						<form:options items="${pFactoryList}" itemLabel="factoryName" itemValue="id" htmlEscape="false"/>
					</form:select>								 
				</c:if>
