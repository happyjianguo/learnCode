 ...
 */
@Service
public class DataUserAnalysisServiceImpl implements DataIUserAnalysisService {
	
	private static final Logger logger = LogManager.getLogger(DataUserAnalysisServiceImpl.class);
	...
	public UserAnalysisTotal QuerryUserAnalysisTotalData(String startDate,String endDate,String appid) {
		...
		String subscriBetime = df.format(new Date());	//当前时间
		...
		if(!StringUtils.isEmpty(appid)){
			...
			analysisTotal = userAnalysisMapper.QuerryUserAnalysisTotal(starttime,endtime,subscriBetime,appid);
...
//		// 查询总关注人数
//		BigDecimal analysisTotal = userAnalysisMapper.QuerryUserAnalysisTotal(starttime,endtime,subscriBetime,appid);
//		userAnalysisTotal.setSubscribeTotal(analysisTotal);
//		// 查询当前正关注人数
//		BigDecimal analysisNow = userAnalysisMapper.QuerryUserAnalysisBytype(starttime,endtime,subscriBetime, subscriPttype,appid);
//		userAnalysisTotal.setSubscribeNow(analysisNow);
//		// 查询当前取消关注的人数
//		subscriPttype = "2";
//		BigDecimal analysisCancle = userAnalysisMapper.QuerryUserAnalysisBytype(starttime,endtime,subscriBetime, subscriPttype,appid);
		userAnalysisTotal.setSubscribeCancle(analysisCancle);
...
//				map.put("man", JsonUtil.obj2json(userAnalysisMapper.IMUserAna("1",appid)));
//				map.put("woman", JsonUtil.obj2json(userAnalysisMapper.IMUserAna("2",appid)));
//				map.put("max", userAnalysisMapper.IMUserAnaMax(appid));
	...
	public List<AuthAccount> getAccount() {
		logger.info("获得有效公众号信息，根据所在组织获取公众号列表");
		Map<String,String> map = new HashMap<String,String>();
		map.put("status","1");
		map.put("userid",AppUtil.getUserDetail().getUserid());
		List<AuthAccount> list = userAnalysisMapper.getDataAccount(map);
		return list;
	}
}
