package cn.yufu.SDMTPlatform.commons;
...
public class Page {
	...
	public Page(int curPage, int sizePerPage, int allRowCount) {
		...
		if (totalPage > 0) {
			if (curPage > totalPage) {
				// modify by zoulin date:2013-09-26
				// throw new IllegalArgumentException();
				// 如果当前页大于总页数，则让当前页为最后一页
				curPage = totalPage;
				this.curPage = curPage;
			}
		}
	...
	public String createPageBar(String url, Map<String, String> param) {
		...
		/*
		 * if(curPage > 1){ pageBarStr.append("<a href=\" " +
		 * urlAndParam.toString()+ "curPage=" + (curPage-1) +"\">上一页</a>");
		 * 
		 * }else{ //pageBarStr.append("<a class=\"disable\">上一页</a>"); }
		 */
		...
		/*
		 * if(curPage < totalPage){ pageBarStr.append("<a href=\" " +
		 * urlAndParam.toString()+ "curPage=" + (curPage+1) +"\">下一页</a>");
		 * 
		 * }else{ //pageBarStr.append("<a class=\"disable\">下一页</a>"); }
		 */
		pageBarStr.append("<a href=\" " + urlAndParam.toString() + "curPage=" + totalPage + "\">末页</a>&nbsp;");
		pageBarStr.append("第" + curPage + "页/共" + totalPage + "页");
		pageBarStr.append(" 共" + allRowCount + "条");
